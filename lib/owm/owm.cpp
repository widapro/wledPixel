...
void owmWeatherUpdate(String city, String unitsFormat, String token) {
  ...
  owmWeatherPostObj.clear();
}

String openWetherMapGetWeather(String whatToDisplay, String unitsFormat) {
  if (whatToDisplay == "owmHumidity") return String(owmHumidity) + "%";
  if (whatToDisplay == "owmPressure") return String(owmPressure);
  if (whatToDisplay == "owmWindSpeed") return String(owmWindSpeed);
  if (whatToDisplay == "owmTemperature") {
    if(unitsFormat == "metric") return String(owmTemperature) + "°C";
    if(unitsFormat == "imperial") return String(owmTemperature) + "°F";
  };
  if (whatToDisplay == "owmWeatherIcon") {
    if(owmWeatherIcon == "01d") return "S";                             // 83   - 'S'   Sun day
    if(owmWeatherIcon == "01n") return "M";                             // 77   - 'M'   Moon night
    if(owmWeatherIcon == "02d") return "s";                             // 115  - 's'   few clouds sun
    if(owmWeatherIcon == "02n") return "m";                             // 109  - 'm'   few clouds night
    if(owmWeatherIcon == "03d" || owmWeatherIcon == "03n") return "c";  // 99   - 'c'   scattered clouds
    if(owmWeatherIcon == "04d" || owmWeatherIcon == "04n") return "C";  // 67   - 'C'   broken clouds
    if(owmWeatherIcon == "09d" || owmWeatherIcon == "09n") return "R";  // 82   - 'R'   shower rain
    if(owmWeatherIcon == "10d" || owmWeatherIcon == "10n") return "r";  // 114  - 'r'   rain
    if(owmWeatherIcon == "11d" || owmWeatherIcon == "11n") return "T";  // 84   - 'T'   thunderstorm
